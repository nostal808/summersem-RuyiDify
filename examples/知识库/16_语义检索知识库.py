...
语义检索依赖 text-embedding 模型。
如果没有配置 Embedding 模型，接口会提示找不到 text-embedding 模型。
...
# 要检索的问题。语义检索更关注“意思相近”，不只是关键词完全匹配。
QUESTION = "怎么把本地资料导入到 Dify 的知识库？"


def retrieve_by_semantic_search() -> dict:
...
def main() -> None:
    print("开始使用语义检索查询 Dify 知识库...")
    print(f"API 地址：{API_BASE_URL}")
    print(f"知识库 ID：{DATASET_ID}")
    print(f"问题：{QUESTION}")

    result = retrieve_by_semantic_search()
    ...
    print("\n说明：语义检索依赖 Embedding 模型，会按问题和内容的含义相似度查找分段。")
...
if __name__ == "__main__":
    try:
        ...
        print(error)
        print("\n可能原因：")
        print("1. Dify 没有配置可用的 text-embedding 模型。")
        print("2. 模型供应商 API Key 没有 Embedding 权限。")
        print("3. 模型供应商账号余额不足，或接口被拒绝。")
